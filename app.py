...
def classify_cuisine_with_gpt(df):
    """Classify restaurants by cuisine using GPT."""
    if df.empty:
        return pd.DataFrame()
    openai.api_key = OPENAI_KEY

    system_msg = """
    أنت مساعد يقوم بتصنيف المطاعم حسب نوع المطبخ (Cuisine).
    اقرأ اسم المطعم، الأنواع (types) ومقتطفات من المراجعات،
    وحدد نوع المطبخ (مثلاً: Italian, American, Indian, Arabic, Fast Food, Seafood, Cafe, Bakery ...).
